...
sub init {
  ...
  $self->npm->install($_) for grep { !$dependencies->{$_}{version} } qw(webpack webpack-cli);
  $self->_render_file('webpack.config.js', $self->config);

  my ($conf_d, @include, %seen) = ($self->assets_dir->child('webpack.config.d'));
  for my $include (@{$self->include}) {
    for my $package (@{$self->dependencies->{$include} || []}) {
      ...
      warn "[Webpack] Template $include_name.js == $has_template\n" if DEBUG;
  ...
  $self->{WEBPACK_INCLUDE} = join ':', @include;
  warn "[Webpack] WEBPACK_INCLUDE=$self->{WEBPACK_INCLUDE}\n" if DEBUG;
...
sub watch {
  ...
  $ENV{WEBPACK_INCLUDE}    = $self->{WEBPACK_INCLUDE};
...
sub _render_file {
  ...
  if (-e $file) {
    ...
    return $self                                                if $version == -1 or $version == $VERSION;
  }
...
sub _run {
  ...
  local $ENV{WEBPACK_INCLUDE}    = $self->{WEBPACK_INCLUDE};
