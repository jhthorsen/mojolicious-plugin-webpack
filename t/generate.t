...
diag 'package.json';
is $asset->_generate($t->app, $base, 'package.json'), 'generated', 'generated package.json';
is $asset->_generate($t->app, $base, 'package.json'), 'custom',    'custom package.json';

diag 'webpack.config.js';
is $asset->_generate($t->app, $base, 'webpack.config.js'), 'generated', 'generated webpack.config.js';
is $asset->_generate($t->app, $base, 'webpack.config.js'), 'current',   'current webpack.config.js';

my $config = $base->child('webpack.config.js')->slurp;
$config =~ s!(Autogenerated.*)\s(\d+\.\d+)!$1 0!;
$base->child('webpack.config.js')->spurt($config);
is $asset->_generate($t->app, $base, 'webpack.config.js'), 'generated', 'regenerated webpack.config.js';

$config =~ s!(Autogenerated.*)!Custom!;
$base->child('webpack.config.js')->spurt($config);
is $asset->_generate($t->app, $base, 'webpack.config.js'), 'custom', 'custom webpack.config.js';

diag 'webpack-custom.js';
$asset->_generate_custom_webpack_hook;
